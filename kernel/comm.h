...
// ... существующие дефайны ...
#define OP_HIJACK_THREAD_ARGS 0x810 
#define OP_GET_TLS_CONTEXT    0x811 // <--- НОВЫЙ ОПКОД
...
// Структура для запроса TLS
typedef struct _TLS_REQUEST {
    pid_t pid;
    uintptr_t base_offset;  // Например 40 (из твоего кода)
    uintptr_t data_offset;  // Например 0x200 (из твоего кода)
    uintptr_t result;       // Сюда вернем адрес
} TLS_REQUEST;
